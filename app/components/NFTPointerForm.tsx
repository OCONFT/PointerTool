...
export function NFTPointerForm() {
  ...
  const buildPointer = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const viewResponse = await viewContract.getCW721Pointer(cwAddress);
      ...
      const executeResponse = await contract.addCW721Pointer(cwAddress);

      await executeResponse.wait();

      const viewResponsePostDeploy = await viewContract.getCW721Pointer(cwAddress);
      ...
      <div className="w-full flex flex-col">
        <form onSubmit={buildPointer}>
          <div className="form-control w-full col-span-full">
            <div className="label">
              <span className="label-text font-semibold text-accent">Collection Address</span>
            </div>
